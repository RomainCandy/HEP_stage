...
class Application(tk.Frame):
    ...
    def make_gui(self):
        ...
        self.buttonS1 = tk.Button(self.top, width=self.width, height=self.height, text='voir stage 1',
                                  command=lambda: self.getStage(1))
        self.buttonS1.pack(side=tk.LEFT)
        
        self.buttonS2 = tk.Button(self.top, width=self.width, height=self.height, text='voir stage 2',
                                  command=lambda: self.getStage(2))
        self.buttonS2.pack(side=tk.LEFT)
        
        self.buttonS3 = tk.Button(self.top, width=self.width, height=self.height, text='voir stage 3',
                                  command=lambda: self.getStage(3))
        self.buttonS3.pack(side=tk.LEFT)
        
        self.middle = tk.Frame(self)
        self.middle.pack(side=tk.TOP, fill=tk.X)
        
        self.buttonS4 = tk.Button(self.middle, width=self.width, height=self.height, text='voir stage 4',
                                  command=lambda: self.getStage(4))
        self.buttonS4.pack(side=tk.LEFT)
        
        self.buttonS5 = tk.Button(self.middle, width=self.width, height=self.height, text='voir stage 5',
                                  command=lambda: self.getStage(5))
        self.buttonS5.pack(side=tk.LEFT)
        
        self.buttonS6 = tk.Button(self.middle, width=self.width, height=self.height, text='voir stage 6',
                                  command=lambda: self.getStage(6))
        ...
        self.buttonL = tk.Button(self.bot, width=self.width, height=self.height, text='charger les données',
                                 command=self.load)
        ...
        tk.Button(self.botbot, width=self.width, height=self.height,
                  text='importer les données pour la premiere utilisation',
    ...
    def importerDonnee(self):
        self.gen = GeneriqueTopLevelListe(self, champ=['liste des stages, fichier .txt', self.choixFile],
                                          selection='multiple')
        self.gen.genbListe.delete(0, tk.END)
        self.gen.bind("<Return>", self.choixFile)
        files = os.listdir('.')
    ...
    def quit(self):
        if not self.hasSaved:
            if askokcancel("Quit", "Des modifications n'ont pas été sauvegardées, quitter quand même?",
                           parent=self, default=tkmsg.OK):
                self.root.destroy()
        else:
...
class FenetreStage(tk.Toplevel):
    ...
    def init_gui(self):
        ...
        self.ajouterForm = tk.Button(self.barreF, text="ajouter formateur",
                                     command=lambda: self.addPersonne('formateur'))
    ...
    def changerAttribution(self):
        ...
        if not len(index):
            showerror(title='probleme choix etudiant',
                      message=" il faut d'abord choisir un étudiant avant de pouvoir permuter ")
        else:
    ...
    def updateContrainte(self):
        if self.changement:
            showerror(title="Changement",
                      message='Il y a eu des modifications depuis la dernière attribution de stage. ' +
            ...
            try:
                ...
                result = askokcancel(title="Confirmation",
                                     message="Toutes les contraintes de l'attribution proposée" +
                                     ' seront ajoutées à tous les étudiants, continuer?')
                if result:
                    self.stage.update(self.numStage)
            except AttributeError as exc:
                showerror(title='Erreur',
                          message="Il faut générer un stage avant de pouvoir l'accepter" + str(exc.args))
    ...
    def genererEdt(self):
        ...
        try:
            ...
            showerror(title='aucune attribution trouvée',
                      message="aucune attribution trouvée, vérifier peut-être les données"
                              " rentrées ou changer de version" + str(exc.args))
        except AttributeError as exc:
        ...
        try:
            for etu in sorted(self.stage.attribution[self.numStage], key=lambda x: str(x)):
                if etu.typeClasse != self.stage.attribution[self.numStage][etu].typeClasse:
                    self.bListeA.insert(tk.END, '-'*50)
                    self.bListeA.insert(tk.END, str(etu.name) + " va avec " +
                                        str(self.stage.attribution[self.numStage][etu].name))
                    self.bListeA.insert(tk.END, '-'*50)
                else:
                    self.bListeA.insert(tk.END, str(etu.name) + " va avec " +
                                        str(self.stage.attribution[self.numStage][etu].name))
...
class InfoPersonne(tk.Toplevel):
    def __init__(self, root, personne, numStage, typePersonne='etudiant'):
        ...
        if typePersonne == 'etudiant':
            ...
            for i in range(2, 4):
                tk.Button(self, text='ajouter', command=lambda i=i: self.modification(i)).grid(row=i+1,
            ...
            tk.Button(self, text='n->n+1', command=lambda: self.changerAnnee('etudiant', lambda x: x+1)).grid(
                row=i+3, column=2, sticky=tk.W)
            tk.Button(self, text='n->n-1', command=lambda: self.changerAnnee('etudiant', lambda x: x-1)).grid(
                row=i+4, column=2, sticky=tk.W)
            ...
            tk.Button(self, text='n->n+1', command=lambda: self.changerAnnee('formateur', lambda x: x+1)).grid(
                row=i+4, column=2, sticky=tk.W)
            tk.Button(self, text='n->n-1', command=lambda: self.changerAnnee('formateur', lambda x: x-1)).grid(
                row=i+5, column=2, sticky=tk.W)
    ...
    def modification(self, i, j=0):
        if i == 0:
            ...
            self.gen = GeneriqueTopLevelListe(self, champ=['ajouter formateur', self.validerAjoutFormateur],
                                              selection='multiple')
            self.pasDejaFormateur = [form for form in self.root.LF if form not in
                                     self.personne.getContrainteFormateur()]
            self.gen.genbListe.delete(0, tk.END)
            ...
            self.gen = GeneriqueTopLevelListe(self, champ=['supprimer formateur', self.validerSupprFormateur],
                                              selection='multiple')
            ...
            self.gen = GeneriqueTopLevelListe(self, champ=['ajouter binome', self.validerAjoutBinome],
                                              selection='multiple')
            self.pasDejaBinome = [etu for etu in self.root.LE if etu not in self.personne.getBinome()
                                  and etu != self.personne]
            self.gen.genbListe.delete(0, tk.END)
            ...
            self.gen = GeneriqueTopLevelListe(self, champ=['supprimer binome', self.validerSupprBinome],
                                              selection='multiple')
